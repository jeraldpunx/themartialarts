<?php
use Carbon\Carbon;
class SchoolOwnerController extends BaseController
{
	// public function home()
	// {
	// 	$page_title = "Home";

	// 	$school_owner = Auth::user();

	// 	$school = $school_owner->school;
	// 	$teachers = Teacher::select('teachers.id', 'teachers.user_id', 'users.first_name', 'users.last_name', 'users.isActive', 'users.picture')
	...
	// 	return View::make('owner.index', compact('page_title', 'school', 'teachers'));		
	// 	// foreach ($tests as $test) {
	// 	// }
	// 	// return $tests;

	// 	// return View::make('owner.index', compact('page_title'));
	...
	// 	// return View::make('instructor.index', compact('page_title', 'students', 'styles'));
	// }
	...
	public function home()
	{
		...
		foreach ($teachers as $teacher) {
			...
			$teacher_students = $teacher_students->orderBy('first_name','ASC')->get()->load('ranks.style')->load('teacher.user');

			if ($i == 0) {
		        $students = $teacher_students;
		        $i++;
		    } else
				$students = $students->merge($teacher_students);
		}

		$school = Auth::user()->school;
		$org_styles = Auth::user()->school->organization->styles;
		$sch_styles = Auth::user()->school->styles;
		$styles = $org_styles->merge($sch_styles);

		return View::make('owner.index', compact('page_title', 'school', 'students', 'styles'));
	}
	...
	public function postInstructor()
	{
		...
		$rules = array(
			...
			'city' 				=> 'required',
			'street' 			=> 'required',
			'username' 			=> 'required|max:255|unique:users,username,'.$input['username'],
		);
		...
		if($validation->passes()) {
			...
			$user = ($input['id']) ? Teacher::find($input['id'])->user : new User;
			...
			if($input['picture_file']) {
				...
				if(!$input['id']) 		$user->picture = 'assets/images/default-user.png';
			}
			...
			$teacher = Teacher::firstOrNew(['user_id' => $user->id, 'school_id' => $school->id]);
			$teacher->user_id 		= $user->id;
			$teacher->school_id 	= $school->id;
			$teacher->save();


			$return['data'] 			= $teacher->load('user');
	...
	public function postChangePasswordInstructor()
	{
		$input = Input::all();
		$rules = array(
		...
		if($validation->passes()) {
			$user = Teacher::find($input['id'])->user;
			$user->password 		= Hash::make($input['password']);
			$user->save();
	...
	public function stylesAndRanks()
	{
		$page_title = "Styles";
		// $styles = DB::SELECT("SELECT * FROM (
		// 				SELECT * 
		...
		return View::make('owner.styles', compact('page_title', 'org_styles', 'sch_styles'));
	}

	public function showStyleRanks()
	{
	...
	public function showInstructorStudents()
	{
		$teacher = Teacher::find(Input::get('teacher_id'));
		return $teacher->students()->orderBy('first_name','ASC')->get();
	}

	public function students($user_id = null)
	{
		if($user_id == null)
		...
		$page_title = "$user->last_name's Students";
		...
		$org_styles = $user->teacher->school->organization->styles;
		$sch_styles = $user->teacher->school->styles;
		$styles = $org_styles->merge($sch_styles);

		return View::make('owner.students', compact('page_title', 'school', 'students', 'styles', 'teacher_user_id'));
	}
	...
	public function postStudent()
	{
		...
		$user = User::find(Input::get('teacher_user_id'));
		$teacher = $user->teacher;
