	...
	<div class="main-content" id="edit-student-profile" style="display: none;">
		...
		<div class="profile-edit profile-view">
			<form id="edit-student-profile-form" enctype="multipart/form-data" autocomplete="off" action="" method="POST" role="form">
				...
				<input type="hidden" value="" name="teacher_user_id">
				<div class="col-lg-3 profile-pic">
	...
	<div class="modal-dialog">
		<form>
			<div class="modal-content">
				...
				<div class="modal-footer">
					<div class="buttons">
						<a href="{{ URL::route('owner_students', $teacher_user_id) }}" class="btn btn-default">Clear Filters</a>
...
var teacher_user_id = '{{ $teacher_user_id }}';
...
$(document).on('click', '#add_student', function(e){
	...
	$div.find("input[name=teacher_user_id]").val(teacher_user_id);
...
$(document).on('click', '#edit_student', function(e){
	...
	view_edit_student_request = $.get('{{ URL::route('owner_json_showStudent') }}', {id: id}).done( function(data){
...
$(document).on('click', '#edit-student-profile #submit_add_or_edit_student', function(e){
	...
	$.ajax({
		url: "{{ URL::route('owner_json_postStudent') }}",  //Server script to process data
		type: 'POST',
...
function updateViewProfile(id) {
	...
	view_student_request = $.get('{{ URL::route('owner_json_showStudent') }}', {id: id}).done( function(data){
...
$(document).on("click", ".delete_student", function(e){
	...
	get_delete_student = $.get('{{ URL::route('owner_json_showStudent') }}', {id: id}).done( function(data){
...
$(document).on("click", "#deleteStudentModal .delete", function(e){
	...
	$.post('{{ URL::route('owner_json_deleteStudent') }}', {id: id}).done( function(obj){
...
$(document).on("click", "#promoteStyleModal .promote", function(e){
	...
	$.post('{{ URL::route('owner_json_promoteStudentStyle') }}', {teacher_user_id: teacher_user_id, studentid: studentid, rankid: rankid}).done( function(obj){
		$this.prop('disabled', false);
		if(obj.flash.type) {
